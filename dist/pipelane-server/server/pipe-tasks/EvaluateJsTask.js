...
exports.EvalJSUtils = {
    ...
    generateUID(input, length = 10) {
        return (0, crypto_1.createHash)("sha256").update(input).digest("hex").substring(0, length);
    },
    generateHashCode(input, maxValue = 1000000000) {
        const safeInput = input ?? '';
        const safeMax = Math.max(1, Math.floor(Number(maxValue) || 0));
        const hashHex = (0, crypto_1.createHash)('sha256').update(safeInput).digest('hex');
...
function testGenerateHashCode() {
    ...
    const maxValue = 5;
    const hash = exports.EvalJSUtils.generateHashCode(input, maxValue);
    ...
    const hashRepeat = exports.EvalJSUtils.generateHashCode(input, maxValue);
