...
class AndroidBot {
    ...
    async shareVideoByFile(filePath, activity, packageName) {
        await this.scanFile(filePath);
        let mediaId = await this.getMediaIdFromPath(filePath);
        console.log("Inserted mediaId", mediaId);
        if (mediaId) {
            await bot.shareVideoById(mediaId, activity, packageName);
        }
        else {
    ...
    async shareVideoById(mediaId, targetActivity, targetPackage) {
        try {
    ...
    async typeText(text, typeNormally = false) {
        if (!typeNormally)
